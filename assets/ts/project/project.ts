/**
 * assets/ts/project/project.ts
 * Module central pour les fonctionnalités de projet
 */

// Types pour le module projet
...
const ProjectModule: ProjectModuleInterface = {
    ...
    announceForScreenReader: function(message: string): void {
    ...
    setupProjectFilters: function(): void {
        // Soumission automatique du formulaire lors du changement de filtre
        const filterForm = document.getElementById('project-filter-form') as HTMLFormElement | null;
    ...
    updateTaskStatus: function(taskId: string, newStatus: string, onSuccess?: (data: any) => void): void {
        ...
        .then(data => {
            ...
            if (taskElement) {
                ...
                const className = taskElement.className.replace(/status-\w+/, `status-${newStatus.toLowerCase()}`);
    ...
    // Interface pour l'ordre des tâches
    updateTaskPosition: function(taskId: string, newColumn: string, taskOrder: Array<{id: string, rank: number}>, onSuccess?: (data: any) => void): void {
        // Créer les données à envoyer
        const data = {
            taskId: taskId,
            newColumn: newColumn,
    ...
    setupTaskManagement: function(): void {
        // Gestion du changement de statut des tâches
        const statusSelects = document.querySelectorAll<HTMLSelectElement>('.task-status-select');
        ...
        const addTaskForm = document.getElementById('add-task-form') as HTMLFormElement | null;
        if (addTaskForm) {
...
// Assigner le module ProjectModule à l'objet window pour une accessibilité globale
